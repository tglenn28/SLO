...
void EmIterativeClosestPoint<N>::align(PointCloudPtr final_cloud,
  ...
  while(converged!=true) {
   ...
   // options.check_gradients = true;
	...
	/////////////////// Attempting to retrieve the covariance /////////////////////////

	ceres::Covariance::Options covOptions;
	ceres::Covariance covariance(covOptions);
	
	std::vector<std::pair<const double*, const double*> > covariance_blocks;
	covariance_blocks.push_back(std::make_pair(est_transform.data(), est_transform.data()));

	CHECK(covariance.Compute(covariance_blocks, &problem));
	
	double covariance_xx[6 * 6];
	//double covariance_xx[Sophus::SE3d::num_parameters * Sophus::SE3d::num_parameters];
	covariance.GetCovarianceBlockInTangentSpace(est_transform.data(), est_transform.data(), covariance_xx);

	std::cout << "Covariance: \n";
	int size = (sizeof(covariance_xx) / sizeof(covariance_xx[0]));
	for (int i = 0; i < size; i++)
	{
		std::cout << covariance_xx[i] << std::endl;
	}
  ...
  std::cout << final_transformation_.matrix() << "\n";
...
void EmIterativeClosestPoint<N>::ComputeCovariances(
  ...
  for(size_t itter = 0; itter < cloudptr->size(); itter++) {
	...
	//dist(pt.label,0) += increment; //Tyler: had to change this because our labels are indexed from 0
