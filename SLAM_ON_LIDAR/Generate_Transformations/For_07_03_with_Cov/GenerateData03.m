...
fileLocation = '~/Desktop/Project/poses/03.txt';
% Find all text files in the following directory
files = dir('~/Desktop/Project/03/*.txt');
...
for iFile=1:numel(files)
    ...
    [T_SICP, T_GICP, covarience] = loadTransformations03(fileWpath);
    edges_sic = [edges_sic; source target reshape(T_SICP.',1,[])];
    edges_gicp = [edges_gicp; source target reshape(T_GICP.',1,[])];
    cov = [cov; source target reshape(covarience.',1,[])];
end
save('transformations&GT.mat','edges_sic','edges_gicp','T_Pose_GT')