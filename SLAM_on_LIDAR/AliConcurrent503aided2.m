...
%%%%%%%%%%%%%%%%%%%%%
% edges_aided4 = edges_non(edges_non(:,size(edges_non,2))==4,:);
% cov_aided4 = cov_non(cov_non(:,size(cov_non,2))==4,:);
% 
% edges_aided3 = edges_non(edges_non(:,size(edges_non,2))==3,:);
% cov_aided3 = cov_non(cov_non(:,size(cov_non,2))==3,:);

edges_aided2 = edges_non(edges_non(:,size(edges_non,2))==2,:);
cov_aided2 = cov_non(cov_non(:,size(cov_non,2))==2,:);

edges_aided2_sicp = edges_sicp_non(edges_sicp_non(:,size(edges_sicp_non,2))==2,:);
cov_aided2_sicp = edges_sicp_non(cov_non(:,size(edges_sicp_non,2))==2,:);

edges_non = edges_non(edges_non(:,size(edges_non,2))>=non_con,:);
cov_non = cov_non(cov_non(:,size(cov_non,2))>=non_con,:);
...
%   Noise = noiseModel.Gaussian.Covariance(reshape(cov_con(currentKey,3:end-1),6,6)');
    ...
    if currentKey <= (length(edges_con)-2)
...
%                 Noise = noiseModel.Gaussian.Covariance(reshape(smoothercov(j,3:end-1),6,6)');
        ...
        if minloop ~= 0 && minloop == currentSKey && count <= maxloopcount
...
saveas(gcf,tempText);
    ...
    function pose3 = findSE3(ICP)
        ...
        import gtsam.*
        pose3 = Pose3(Tr * reshape(ICP(1,3:18),4,4)' * inv(Tr));
